...
const tableData = ref({ alternative: [] });
...
const fetchItems = async () => {
    try {
        ...
        tableData.value = {
            alternative: Array.isArray(response.data.data?.alternative)
                ? response.data.data.alternative
                : [],
        };
    } catch (error) {
        console.error('Error fetching items:', error);
        tableData.value = { alternative: [] };
    }
};
        ...
        <div role="tablist" class="tabs-box tabs">
            <div class="max-h-[80vh] w-full overflow-x-auto p-4">
                <div class="flex flex-col gap-6">
                    <AccentButton
                        @click="openModal('alternative', 'add', null)"
                    >
                        Add Alternatif
                    </AccentButton>
                    <table class="table-pin-rows table-pin-cols table w-full">
